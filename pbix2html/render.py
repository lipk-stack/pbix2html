...
from pbix2html.reader import Page, PbixReport, Relationship, Visual
...
.measure { margin: 0 0 .3em; }
.measure:last-child { margin-bottom: 0; }
.measure-expr { display: block; color: var(--muted); font-family: ui-monospace, SFMono-Regular, Menlo, Consolas, monospace; font-size: .82em; white-space: pre-wrap; word-break: break-word; }
...
def _render_measures(table_measures: list) -> str:
    if not table_measures:
        return "&mdash;"
    parts = []
    for measure in table_measures:
        expr = f'<span class="measure-expr">{_esc(measure.expression)}</span>' if measure.expression else ""
        parts.append(f'<div class="measure"><code>{_esc(measure.name)}</code>{expr}</div>')
    return "".join(parts)
...
def _render_tables(report: PbixReport) -> str:
    ...
    for table in report.tables:
        rows.append(
            ...
            f"<td>{_render_measures(table.measures)}</td>"
            "</tr>"
        )
    return (
...
def _render_relationships(report: PbixReport) -> str:
    if not report.relationships:
        return ""
    rows = []
    for rel in report.relationships:  # type: Relationship
        direction = "Both directions" if rel.cross_filter_direction == "both" else "Single direction"
        status = "Active" if rel.active else '<span class="badge">inactive</span>'
        rows.append(
            "<tr>"
            f"<td>{_esc(rel.from_table)}.{_esc(rel.from_column)}</td>"
            f"<td>{_esc(rel.to_table)}.{_esc(rel.to_column)}</td>"
            f"<td>{_esc(direction)}</td>"
            f"<td>{status}</td>"
            "</tr>"
        )
    return (
        '<section class="card-section"><h2>Relationships</h2><div class="card"><table>'
        "<thead><tr><th>From</th><th>To</th><th>Cross-filter</th><th>Status</th></tr></thead>"
        f"<tbody>{''.join(rows)}</tbody></table></div></section>"
    )
...
def render_html(report: PbixReport) -> str:
    ...
    summary = (
        ...
        f"<li><strong>{len(report.relationships)}</strong> relationships</li>"
    ...
    return (
        ...
        "</div></header>"
        f"<main>{fidelity}{pages}{_render_tables(report)}{_render_relationships(report)}{_render_resources(report)}</main>"
